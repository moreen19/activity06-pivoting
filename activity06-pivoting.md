...
**Response**: by\_speaker is untidy because it has the seasons variable
in every column and the count in every row. We could solve this by
putting the seasons into one column and the word count into one column.
...
I would summarise the values for each season and the values for the bec
...
``` r
pivot_wider(friends_tidy, names_from = speaker, values_from = words )
```

    ## Error: Can't subset columns that don't exist.
    ## x Columns `a`, `able`, `about`, `absolute`, `accept`, etc. don't exist.
